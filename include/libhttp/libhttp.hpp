#ifndef LIBHTTP_LIBHTTP_HPP
#define LIBHTTP_LIBHTTP_HPP
...
namespace http
{
    inline Response request_once(const std::string &method, const std::string &url, RequestOptions options = {})
    {
        ...
        {
            options.headers["User-Agent"] = "libhttp/3.0";
        }
        ...
        if (!options.auth_type.empty() && !options.auth_token.empty() && options.headers.find("Authorization") == options.headers.end())
        {
            if (options.auth_type == "bearer")
            {
                options.headers["Authorization"] = detail::build_bearer_auth_header(options.auth_token);
            }
            else
            {
                options.headers["Authorization"] = detail::build_custom_auth_header(options.auth_type, options.auth_token);
            }
        }
